...
Все семейства подавляющих комментариев работают для одной строки и диапазона:
маркер после кода действует на текущую строку, а маркер на отдельной строке
открывает диапазон до `noqa-enable`, `bsl-enable` или совместимого `BSLLS:…-on`.
